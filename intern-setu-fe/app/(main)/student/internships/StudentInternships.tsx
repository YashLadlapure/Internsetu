...
import { selectStudentInternships, setStudentInternships } from "@/lib/features/student/studentSlice";
...
const StudentInternships = () => {
    ...
    const internships = useAppSelector(selectStudentInternships);
    const [selectedInternship, setSelectedInternship] = useState<number | string | null>(null);
    ...
    const filteredInternships = useMemo(
        ...
        }, [internships, locationFilter]
    )

    const fetchInternships = async () => {
        try {
            const res = await callBackend("/student/internship");
            if(!res.success) throw new Error("Failed to fetch internships");
            dispatch(setStudentInternships(res.data));
        } catch (error) {
            console.log(error);
        }
    }
                            ...
                            <Card key={internship.id} className="flex flex-col group h-full border border-neutral-200 dark:border-neutral-800 hover:border-neutral-400 transition-all">
                                <div className="p-5 flex flex-col h-full">
                                    ...
                                    <div className="mt-4">
                                        ...
                                        >
                                            {internship.hasApplied ? "Already Applied" : "Apply"}
                                        </Button>
                                    </div>
                                </div>
                            </Card>
